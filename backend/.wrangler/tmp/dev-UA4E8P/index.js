...
// .wrangler/tmp/bundle-mgMWxw/checked-fetch.js
...
app.post("/auth/register", async (c) => {
  ...
  try {
    await DB.prepare("INSERT INTO users (name, email, password) VALUES (?, ?, ?)").bind(name, email, password).run();
    return c.json({ success: true, message: "Conta criada com sucesso!" });
  } catch (e) {
    return c.json({ success: false, message: "Este e-mail j\xE1 est\xE1 em uso." }, 400);
  }
});
...
app.get("/admin/stats", async (c) => {
  ...
  try {
    ...
    console.error("Erro ao buscar estat\xEDsticas:", e);
...
app.post("/restaurant/create", async (c) => {
  ...
  if (!email) return c.json({ success: false, message: "N\xE3o autenticado." }, 401);
  ...
  try {
    ...
    return c.json({ success: false, message: "Esse nome j\xE1 est\xE1 em uso." }, 409);
  }
});
...
// .wrangler/tmp/bundle-mgMWxw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-mgMWxw/middleware-loader.entry.ts
